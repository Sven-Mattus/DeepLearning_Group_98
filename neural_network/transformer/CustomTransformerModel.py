...
class CustomTransformerModel(tf.keras.Model):
    ...
    def call(self, input_idx):  # input_ind of size nr_seq x SEQ_LENGTH
        pos = tf.range(self.seq_length)[tf.newaxis, :]
        input_emb = self.lay_input_embed(input_idx)  # shape (b, t, n_embd)
        pos_emb = self.lay_pos_embed(pos)  # (t, n_embd)
...
class CausalSelfAttention(layers.Layer):
    ...
    def __init__(self, dropout_rate, nr_heads, embedding_dim):
        ...
        # key, query, value projections for all heads, but together in a batch
        self.lay_attn = layers.Dense(units=3 * embedding_dim, use_bias=False)  # todo überall bias = False ?
